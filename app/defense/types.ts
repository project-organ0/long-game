...
export interface OrganConfig {
  id: OrganType; name: string; role: string; color: string;
...
export interface EnemyConfig {
  id: EnemyType; name: string; color: string;
  maxHp: number; speed: number; reward: number;
  // 특수 행동 플래그
...
export interface PlacedTowerState {
  id: number; type: TowerType; slot: number; level: number;
  affinity: OrganType;
  canEvolve: boolean;
}
